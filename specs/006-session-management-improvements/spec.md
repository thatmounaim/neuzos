...
- EC-02: ACK timing — the delete-flow ACK (`event.stop_session_ack`) MUST be sent only after Svelte's `tick()` resolves so the `<webview>` element is confirmed removed from the DOM before the grace period starts; a premature ACK can leave Chromium holding file handles when rimraf runs (see BUG-013). Note: DOM removal alone is not sufficient — Chromium's internal WebContents teardown and LevelDB handle release are deferred and can take an additional 2–5 s on Windows after the DOM flush, which is why the grace period itself must be 5 s (see BUG-014 and FR-003).
...
**Bugfix**: 2026-04-26 — BUG-014: FR-003 grace period updated 2s→5s; FR-004 retry logic updated (8×1200ms + existsSync check + rimraf maxRetries); EC-02 extended with LevelDB recreation note.
